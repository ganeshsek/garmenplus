...
class mpackingmaterialmodel extends CI_Model {
    ...
    function savePackingMaterialInfo($ArrUpdateData){
        ...
        if($VarId=="") {
            ...
            if(@$ArrCheckExist['id']=='' && $ArrUpdateData['packingmaterialname']<>'') {
                ...
                $ArrResult['errcode']						    = '-1';
                $ArrResult['msg']							    = "This packing material has already Exist!";
            }
        }
        return $ArrResult;
    }
    ...
    function fnDelPackingMaterial($VarSizeId='',$VarUpdatedBy='') {
        if(@$VarSizeId>=1) {
            $ArrUpdateData          = array('status'=>3,'dateupdated'=>date('Y-m-d H:i:s'),'updatedby'=>$VarUpdatedBy);
            if($this->db->update(KN_MASTER_PACKING_MATERIAL,$ArrUpdateData,array('id'=>$VarSizeId))) {
                $ArrResult['errcode']					    = 1;
                $ArrResult['msg']							= '';
