...
class mtrimmingtype extends CI_Controller {

    private $limit = 10;
    public function __construct() {
        parent::__construct();
        error_reporting(0);
        $this->load->model(CNFCOMPANY."/mtrimmingtypemodel");
    ...
    public function addedit() {
        ...
        if($VarId<>'' && base64_decode(urldecode($VarId))) {
            ...
            $ArrResults     									= $this->mtrimmingtypemodel->fnGetTrimmingTypeInfo('','',$VarUserId);
            $ArrData['ArrBasicInfo']	    				    = $ArrResults[0];
            $ArrData['VarId']					                = $ArrResults[0]['id'];
        } else {
            $ArrData['VarNew']  							    = 1;
        }
        $this->load->view(CNFCOMPANY.'/addedittrimmingtype',$ArrData);
    }
    ...
    public function updateTrimmingInfo() {
        ...
        if($VarTrimmingName<>'') {
            ...
            $ArrStatus                                          = $this->mtrimmingtypemodel->saveTrimmingTypeInfo($ArrUpdateData);
    ...
    public function managetrimmingtype() {
        ...
        if($VarFrom==1) {
            ...
            $config['base_url']									= base_url().CNFCOMPANY.'mtrimmingtype/managetrimmingtype/';
            $config['total_rows']								= $this->mtrimmingtypemodel->fnCountTrimmingType($VarTrimmingName,$VarStatus);
            $config['per_page']									= 10;
            ...
            $sortby												= "dateupdated";
            $sortorder											= "desc";
            $ArrTrimmingTypeList								= $this->mtrimmingtypemodel->fnListTrimmingType($VarTrimmingName,$VarStatus,$this->limit,$offset,$sortby,$sortorder)->result();
            $data['pagination']									= $this->pagination->create_linkswithajax('TrimmingType');
            $i=0;
            $ArrFnlList										    = array();
            $ArrStatus                                          = unserialize(ARRSTATUS);
            foreach ($ArrTrimmingTypeList as $ObjUnit){
    ...
    function delTrimmingTypeInfo() {
        $VarUnitId                                            = xssclean($this->input->post('id'));
        $VarUpdatedBy                                         = fnGetUserLoggedInfo();
        if($VarUnitId>=1) {
            $ArrResult             					          = $this->mtrimmingtypemodel->fnDelTrimmingType($VarUnitId,$VarUpdatedBy);
        } else {
            $ArrResult['errcode']						      = -1;
            $ArrResult['msg']							      = '';
        }
