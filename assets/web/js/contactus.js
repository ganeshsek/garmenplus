function fnValidateContactUs() {
    ...
    var CustomerName		    				= $("#frmHelpName").val();
    var EmailId		    						= $("#frmHelpEmail").val();
    var Mobile      							= $("#frmHelpMobile").val();
    ...
    if(jsTrim(CustomerName)== ""){
        $('#ErrHelpName').html("Please fill the Name");
        $('#frmHelpName').focus();
        $('#frmHelpName').css("border", "1px solid #B94A48");
        return false;
    }
    if(jsTrim(EmailId)== ""){
        $('#ErrHelpEmail').html("Please fill the Title");
        $('#frmHelpEmail').focus();
        $('#frmHelpEmail').css("border", "1px solid #B94A48");
        return false;
    } else if(!isEmail(EmailId)) {
        $('#ErrHelpEmail').html("Please fill the Vaild E-Mail Id!!!");
        $('#frmHelpEmail').focus();
        $('#frmHelpEmail').css("border", "1px solid #B94A48");
        return false;
    }
    if(jsTrim(Mobile)== ""){
        $('#ErrHelpMobile').html("Please fill the Mobile Number");
        $('#frmHelpMobile').focus();
        $('#frmHelpMobile').css("border", "1px solid #B94A48");
        return false;
    }
    if (window.FormData){
        ProfileFormData								= new FormData();
        ProfileFormData.append("qf",$("#frmHelpAttach").prop("files")[0]);
        ProfileFormData.append("tn",TopicName);
        ProfileFormData.append("t",Title);
        ProfileFormData.append("msg",Message);
        ProfileFormData.append("cn",CustomerName);
        ProfileFormData.append("e",EmailId);
        ProfileFormData.append("m",Mobile);
    }
    $.ajax({
        url 		: base_path+'contact/sendRequestContactUs',
        ...
        success     : function(data, textStatus, jqXHR){
            ...
            fnValidateContactUsRes(data);
        }
    });
    return false;
}

function fnValidateContactUsRes(data){
    if(data!=''){
        if(data.errcode == '404') {
            fnCallSessionExpire();
            return false;
            ...
            $("#divSuccessContactInfoMsg").html("Thanks for contacting us!.<br><br>Your Ticket No is <b>"+data.tc+"</b>. <br> We Will Solve your Query Within 1-2 Business Working Days.");
            $("#divContachfrm").addClass('hide');
...
function fnSearchTicketInfo() {
    ...
    MakePostRequest(base_path+'userqueries/manageticket',GlbSearchParam,'json',fnListAllTicketRes);
}
                ...
                if(data.cn>0) {
                    ...
                    if(data.ct>0) {
                        $.each(data.re,function(index,value){
                            ...
                            PageContent=PageContent+'<a href="'+base_path+'userqueries/ticketdetails/'+value.id+'/"><i class="fa fa-file-text-o"></i> View Status</a>&nbsp;&nbsp;';
...
function fnSaveTicketRepliedMsg() {
    ...
    $.ajax({
        url 		: base_path+'userqueries/sendRepliedQueries',
        data        : ProfileFormData ? ProfileFormData : ObjForm.serialize(),
        cache       : false,
        contentType : false,
        processData : false,
...
function fnShowTicketRepliedInfo(RepliedId,UserId) {
    ...
    MakePostRequest(base_path+"userqueries/GetQueriesRepliedDetails/",Parameters,'json',fnShowTicketRepliedInfoRes);
}
