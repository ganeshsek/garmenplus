...
function fnSearchPackingCode() {
    ...
    MakePostRequest(base_path+GlbCAdminFdr+'/mpackingcode/managepackingcode',GlbSearchParam,'json',fnListPackingCodeRes);
}
                ...
                if(data.cn>0) {
                    ...
                    if(data.ct>0) {
                        $.each(data.re,function(index,value){
                            PageContent=PageContent+'<tr><td><a href="'+base_path+GlbCAdminFdr+'/mpackingcode/addedit/'+escape(base64_encode(value.id))+'/">'+value.n+'</a></td><td>'+value.s+'</td><td>'+value.ub+'</td><td>'+value.du+'</td><td><i class="fa fa-trash-o"></i>&nbsp;&nbsp;<a href="javascript:void(0);" onclick="fnDeletePackingCode('+value.id+')">Delete</a></td>';
...
function fnDeletePackingCode(Id) {
    if(confirm("Are you want to delete this packing code?")) {
        ...
        MakePostRequest(base_path+GlbCAdminFdr+'/mpackingcode/delPackingCodeInfo',Parameters,'json',fnDeletePackingCodeRes);
    }
}
...
function fnSavePackingCodeInfo() {
    try{
        ...
        $.ajax({
            url 		: base_path+GlbCAdminFdr+'/mpackingcode/updatePackingCodeInfo',
...
function fnSavePackingCodeRes(data){
    if(data!=''){
        if(data.errcode == '404') {
            ...
            fnRedirectPageTimeOut(base_path+GlbCAdminFdr+'/mpackingcode/addedit/'+data.eid);
        }
    }
}