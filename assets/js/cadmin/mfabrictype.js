...
function fnSearchFabricType() {
    ...
    MakePostRequest(base_path+GlbCAdminFdr+'/mfabrictype/managefabrictypes',GlbSearchParam,'json',fnListFabricTypeRes);
}
                ...
                if(data.cn>0) {
                    ...
                    if(data.ct>0) {
                        $.each(data.re,function(index,value){
                            PageContent=PageContent+'<tr><td><a href="'+base_path+GlbCAdminFdr+'/mfabrictype/addedit/'+escape(base64_encode(value.id))+'/">'+value.n+'</a></td><td>'+value.s+'</td><td>'+value.ub+'</td><td>'+value.du+'</td><td><i class="fa fa-trash-o"></i>&nbsp;&nbsp;<a href="javascript:void(0);" onclick="fnDeleteFabricType('+value.id+')">Delete</a></td>';
...
function fnDeleteFabricType(Id) {
    if(confirm("Are you want to delete this fabric type?")) {
        ...
        MakePostRequest(base_path+GlbCAdminFdr+'/mfabrictype/delFabricTypeInfo',Parameters,'json',fnDeleteFabricTypeRes);
    }
}
...
function fnSaveFabricTypeInfo() {
    try{
        ...
        $.ajax({
            url 		: base_path+GlbCAdminFdr+'/mfabrictype/updateFabricInfo',
...
function fnSaveFabricTypeRes(data){
    if(data!=''){
        if(data.errcode == '404') {
            ...
            fnRedirectPageTimeOut(base_path+GlbCAdminFdr+'/mfabrictype/addedit/'+data.eid);
        }
    }
}