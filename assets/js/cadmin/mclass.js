...
function fnSearchClass() {
    ...
    MakePostRequest(base_path+GlbCAdminFdr+'/mclass/manageclass',GlbSearchParam,'json',fnListClassRes);
}
                ...
                if(data.cn>0) {
                    ...
                    if(data.ct>0) {
                        $.each(data.re,function(index,value){
                            PageContent=PageContent+'<tr><td><a href="'+base_path+GlbCAdminFdr+'/mclass/addedit/'+escape(base64_encode(value.id))+'/">'+value.n+'</a></td><td>'+value.s+'</td><td>'+value.ub+'</td><td>'+value.du+'</td><td><i class="fa fa-trash-o"></i>&nbsp;&nbsp;<a href="javascript:void(0);" onclick="fnDeleteClass('+value.id+')">Delete</a></td>';
...
function fnDeleteClass(Id) {
    if(confirm("Are you want to delete this class?")) {
        ...
        MakePostRequest(base_path+GlbCAdminFdr+'/mclass/delClassInfo',Parameters,'json',fnDeleteClassRes);
    }
}
...
function fnSaveDPFInfo() {
    try{
        ...
        $.ajax({
            url 		: base_path+GlbCAdminFdr+'/mclass/updateClassInfo',
...
function fnSaveClassRes(data){
    if(data!=''){
        if(data.errcode == '404') {
            ...
            fnRedirectPageTimeOut(base_path+GlbCAdminFdr+'/mclass/addedit/'+data.eid);
        }
    }
}