...
function fnSearchDSR() {
    ...
    MakePostRequest(base_path+GlbCAdminFdr+'/mdsr/managedsr',GlbSearchParam,'json',fnListDSRRes);
}
                ...
                if(data.cn>0) {
                    ...
                    if(data.ct>0) {
                        $.each(data.re,function(index,value){
                            PageContent=PageContent+'<tr><td><a href="'+base_path+GlbCAdminFdr+'/mdsr/addedit/'+escape(base64_encode(value.id))+'/">'+value.n+'</a></td><td>'+value.s+'</td><td>'+value.ub+'</td><td>'+value.du+'</td><td><i class="fa fa-trash-o"></i>&nbsp;&nbsp;<a href="javascript:void(0);" onclick="fnDeleteDSR('+value.id+')">Delete</a></td>';
...
function fnDeleteDSR(Id) {
    if(confirm("Are you want to delete this dyeing special request?")) {
        ...
        MakePostRequest(base_path+GlbCAdminFdr+'/mdsr/delDSRInfo',Parameters,'json',fnDeleteDSRRes);
    }
}
...
function fnSaveDSRInfo() {
    try{
        ...
        $.ajax({
            url 		: base_path+GlbCAdminFdr+'/mdsr/updateDSRInfo',
...
function fnSaveDSRRes(data){
    if(data!=''){
        if(data.errcode == '404') {
            ...
            fnRedirectPageTimeOut(base_path+GlbCAdminFdr+'/mdsr/addedit/'+data.eid);
        }
    }
}