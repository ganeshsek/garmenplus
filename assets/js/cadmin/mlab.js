...
function fnSearchLab() {
    ...
    MakePostRequest(base_path+GlbCAdminFdr+'/mlab/managelab',GlbSearchParam,'json',fnListLabRes);
}
                ...
                if(data.cn>0) {
                    ...
                    if(data.ct>0) {
                        $.each(data.re,function(index,value){
                            PageContent=PageContent+'<tr><td><a href="'+base_path+GlbCAdminFdr+'/mlab/addedit/'+escape(base64_encode(value.id))+'/">'+value.n+'</a></td><td>'+value.s+'</td><td>'+value.ub+'</td><td>'+value.du+'</td><td><i class="fa fa-trash-o"></i>&nbsp;&nbsp;<a href="javascript:void(0);" onclick="fnDeleteLab('+value.id+')">Delete</a></td>';
...
function fnDeleteLab(Id) {
    if(confirm("Are you want to delete this lab?")) {
        ...
        MakePostRequest(base_path+GlbCAdminFdr+'/mlab/delLabInfo',Parameters,'json',fnDeleteLabRes);
    }
}
...
function fnSaveLabInfo() {
    try{
        ...
        $.ajax({
            url 		: base_path+GlbCAdminFdr+'/mlab/updateLabInfo',
...
function fnSaveLabRes(data){
    if(data!=''){
        if(data.errcode == '404') {
            ...
            fnRedirectPageTimeOut(base_path+GlbCAdminFdr+'/mlab/addedit/'+data.eid);
        }
    }
}