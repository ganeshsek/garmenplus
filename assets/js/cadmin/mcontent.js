...
function fnSearchContent() {
    ...
    MakePostRequest(base_path+GlbCAdminFdr+'/mcontent/managecontent',GlbSearchParam,'json',fnListContentRes);
}
                ...
                if(data.cn>0) {
                    ...
                    if(data.ct>0) {
                        $.each(data.re,function(index,value){
                            PageContent=PageContent+'<tr><td><a href="'+base_path+GlbCAdminFdr+'/mcontent/addedit/'+escape(base64_encode(value.id))+'/">'+value.n+'</a></td><td>'+value.s+'</td><td>'+value.ub+'</td><td>'+value.du+'</td><td><i class="fa fa-trash-o"></i>&nbsp;&nbsp;<a href="javascript:void(0);" onclick="fnDeleteContent('+value.id+')">Delete</a></td>';
...
function fnDeleteContent(Id) {
    if(confirm("Are you want to delete this content?")) {
        ...
        MakePostRequest(base_path+GlbCAdminFdr+'/mcontent/delContentInfo',Parameters,'json',fnDeleteContentRes);
    }
}
...
function fnSaveContentInfo() {
    try{
        ...
        $.ajax({
            url 		: base_path+GlbCAdminFdr+'/mcontent/updateContentInfo',
...
function fnSaveContentRes(data){
    if(data!=''){
        if(data.errcode == '404') {
            ...
            fnRedirectPageTimeOut(base_path+GlbCAdminFdr+'/mcontent/addedit/'+data.eid);
        }
    }
}