...
function fnSearchPrintType() {
    ...
    MakePostRequest(base_path+GlbCAdminFdr+'/mprinttype/manageprinttype',GlbSearchParam,'json',fnListPrintTypeRes);
}
                ...
                if(data.cn>0) {
                    ...
                    if(data.ct>0) {
                        $.each(data.re,function(index,value){
                            PageContent=PageContent+'<tr><td><a href="'+base_path+GlbCAdminFdr+'/mprinttype/addedit/'+escape(base64_encode(value.id))+'/">'+value.n+'</a></td><td>'+value.s+'</td><td>'+value.ub+'</td><td>'+value.du+'</td><td><i class="fa fa-trash-o"></i>&nbsp;&nbsp;<a href="javascript:void(0);" onclick="fnDeletePrintType('+value.id+')">Delete</a></td>';
...
function fnDeletePrintType(Id) {
    if(confirm("Are you want to delete this print type?")) {
        ...
        MakePostRequest(base_path+GlbCAdminFdr+'/mprinttype/delPrintTypeInfo',Parameters,'json',fnDeletePrintTypeRes);
    }
}
...
function fnSavePrintTypeInfo() {
    try{
        ...
        $.ajax({
            url 		: base_path+GlbCAdminFdr+'/mprinttype/updatePrintInfo',
...
function fnSavePrintTypeRes(data){
    if(data!=''){
        if(data.errcode == '404') {
            ...
            fnRedirectPageTimeOut(base_path+GlbCAdminFdr+'/mprinttype/addedit/'+data.eid);
        }
    }
}