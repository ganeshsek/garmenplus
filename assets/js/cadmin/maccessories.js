...
function fnSearchAccessories() {
    ...
    MakePostRequest(base_path+GlbCAdminFdr+'/maccessories/manageaccessories',GlbSearchParam,'json',fnListAccessoriesRes);
}
                ...
                if(data.cn>0) {
                    ...
                    if(data.ct>0) {
                        $.each(data.re,function(index,value){
                            PageContent=PageContent+'<tr><td><a href="'+base_path+GlbCAdminFdr+'/maccessories/addedit/'+escape(base64_encode(value.id))+'/">'+value.n+'</a></td><td>'+value.s+'</td><td>'+value.ub+'</td><td>'+value.du+'</td><td><i class="fa fa-trash-o"></i>&nbsp;&nbsp;<a href="javascript:void(0);" onclick="fnDeleteAccessories('+value.id+')">Delete</a></td>';
...
function fnDeleteAccessories(Id) {
    if(confirm("Are you want to delete this accessories?")) {
        ...
        MakePostRequest(base_path+GlbCAdminFdr+'/maccessories/delAccessoriesInfo',Parameters,'json',fnDeleteAccessoriesRes);
    }
}
...
function fnSaveAccessoriesInfo() {
    try{
        ...
        $.ajax({
            url 		: base_path+GlbCAdminFdr+'/maccessories/updateAccessoriesInfo',
...
function fnSaveAccessoriesRes(data){
    if(data!=''){
        if(data.errcode == '404') {
            ...
            fnRedirectPageTimeOut(base_path+GlbCAdminFdr+'/maccessories/addedit/'+data.eid);
        }
    }
}