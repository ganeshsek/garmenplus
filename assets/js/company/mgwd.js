...
function fnSearchGWD() {
    ...
    MakePostRequest(base_path+GlbCompanyFdr+'/mgwd/managegwd',GlbSearchParam,'json',fnListGWDRes);
}
                ...
                if(data.cn>0) {
                    ...
                    if(data.ct>0) {
                        $.each(data.re,function(index,value){
                            PageContent=PageContent+'<tr><td><a href="'+base_path+GlbCompanyFdr+'/mgwd/addedit/'+escape(base64_encode(value.id))+'/">'+value.n+'</a></td><td>'+value.s+'</td><td>'+value.ub+'</td><td>'+value.du+'</td><td><i class="fa fa-trash-o"></i>&nbsp;&nbsp;<a href="javascript:void(0);" onclick="fnDeleteGWD('+value.id+')">Delete</a></td>';
...
function fnDeleteGWD(Id) {
    if(confirm("Are you want to delete this garment wash?")) {
        ...
        MakePostRequest(base_path+GlbCompanyFdr+'/mgwd/delGWDInfo',Parameters,'json',fnDeleteGWDRes);
    }
}
...
function fnSaveGWDInfo() {
    try{
        ...
        $.ajax({
            url 		: base_path+GlbCompanyFdr+'/mgwd/updateGWDInfo',
...
function fnSaveGWDRes(data){
    if(data!=''){
        if(data.errcode == '404') {
            ...
            fnRedirectPageTimeOut(base_path+GlbCompanyFdr+'/mgwd/addedit/'+data.eid);
        }
    }
}