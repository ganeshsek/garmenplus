        ...
        if(data.errcode!=undefined) {
            if(data.errcode == '404') {
                ...
                if(data.cn>0) {
                    ...
                    PageContent	= "<table class='table table-bordered table-hover'><thead><tr><th>Unit of Measure</th><th>Status</th><th>Updated By</th><th>Date Updated</th><th>Action</th></tr></thead><tbody>";
                    if(data.ct>0) {
                        $.each(data.re,function(index,value){
                            PageContent=PageContent+'<tr><td><a href="'+base_path+GlbCompanyFdr+'/munitmeasure/addedit/'+escape(base64_encode(value.id))+'/">'+value.n+'</a></td><td>'+value.s+'</td><td>'+value.ub+'</td><td>'+value.du+'</td><td><i class="fa fa-trash-o"></i>&nbsp;&nbsp;<a href="javascript:void(0);" onclick="fnDeleteUnitMeasure('+value.id+')">Delete</a></td>';
...
function fnSaveUnitMeasureInfo() {
    try{
        ...
        if(jsTrim(UnitName)== ""){
            $('#ErrBasicUnitName').html("Please fill the unit of measure");
...
function fnSaveUnitMeasureRes(data){
    if(data!=''){
        if(data.errcode == '404') {
            ...
            $("#divSuccessBasicInfoMsg").html("Unit measure has updated at successfully!");
