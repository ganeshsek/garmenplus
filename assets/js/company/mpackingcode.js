...
function fnSearchPackingCode() {
    ...
    MakePostRequest(base_path+GlbCompanyFdr+'/mpackingcode/managepackingcode',GlbSearchParam,'json',fnListPackingCodeRes);
}
        ...
        if(data.errcode!=undefined) {
            if(data.errcode == '404') {
                ...
                if(data.cn>0) {
                    ...
                    PageContent	= "<table class='table table-bordered table-hover'><thead><tr><th>PackingCode</th><th>Status</th><th>Updated By</th><th>Date Updated</th><th>Action</th></tr></thead><tbody>";
                    if(data.ct>0) {
                        $.each(data.re,function(index,value){
                            PageContent=PageContent+'<tr><td><a href="'+base_path+GlbCompanyFdr+'/mpackingcode/addedit/'+escape(base64_encode(value.id))+'/">'+value.n+'</a></td><td>'+value.s+'</td><td>'+value.ub+'</td><td>'+value.du+'</td><td><i class="fa fa-trash-o"></i>&nbsp;&nbsp;<a href="javascript:void(0);" onclick="fnDeletePackingCode('+value.id+')">Delete</a></td>';
...
function fnPaginationPackingCode(VarURL) {
    ...
    $("#DivTotalCntResult").html('');
    $("#ResResult").html("<img src='"+base_path+"/assets/img/loader.gif' height='8' style='padding-left:10px'>");
    MakePostRequest(VarURL,Parameters,'json',fnListPackingCodeRes);
}

function fnDeletePackingCode(Id) {
    if(confirm("Are you want to delete this packing code?")) {
        var Parameters = "id="+Id;
        MakePostRequest(base_path+GlbCompanyFdr+'/mpackingcode/delPackingCodeInfo',Parameters,'json',fnDeletePackingCodeRes);
    }
}
...
function fnSavePackingCodeInfo() {
    try{
        ...
        if(jsTrim(PackingCode)== ""){
            $('#ErrBasicPackingCode').html("Please fill the packing code");
...
function fnSavePackingCodeRes(data){
    if(data!=''){
        if(data.errcode == '404') {
            ...
            $('#ErrBasicPackingCode').html(data.msg);
            return false;
        } else if(data.errcode==1){
            GlbId       = data.id;
            $("#divSuccessBasicInfoMsg").removeClass('hide');
            $("#divSuccessBasicInfoMsg").html("Packing code has updated at successfully!");
