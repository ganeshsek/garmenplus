...
function fnSearchDyeingMethod() {
    ...
    MakePostRequest(base_path+GlbCompanyFdr+'/mdyeingmethod/managedyeingmethod',GlbSearchParam,'json',fnListDyeingMethodRes);
}
                ...
                if(data.cn>0) {
                    ...
                    if(data.ct>0) {
                        $.each(data.re,function(index,value){
                            PageContent=PageContent+'<tr><td><a href="'+base_path+GlbCompanyFdr+'/mdyeingmethod/addedit/'+escape(base64_encode(value.id))+'/">'+value.n+'</a></td><td>'+value.s+'</td><td>'+value.ub+'</td><td>'+value.du+'</td><td><i class="fa fa-trash-o"></i>&nbsp;&nbsp;<a href="javascript:void(0);" onclick="fnDeleteDyeingMethod('+value.id+')">Delete</a></td>';
...
function fnDeleteDyeingMethod(Id) {
    if(confirm("Are you want to delete this dyeing method?")) {
        ...
        MakePostRequest(base_path+GlbCompanyFdr+'/mdyeingmethod/delDeyingMethodInfo',Parameters,'json',fnDeleteDyeingMethodRes);
    }
}
...
function fnSaveDyeingMethodInfo() {
    try{
        ...
        $.ajax({
            url 		: base_path+GlbCompanyFdr+'/mdyeingmethod/updateDyeingInfo',
...
function fnSaveDyeingMethodRes(data){
    if(data!=''){
        if(data.errcode == '404') {
            ...
            fnRedirectPageTimeOut(base_path+GlbCompanyFdr+'/mdyeingmethod/addedit/'+data.eid);
        }
    }
}