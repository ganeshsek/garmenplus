...
function fnSearchLab() {
    var LabName          		                = $("#frmSrchLabName").val();
    ...
    MakePostRequest(base_path+GlbCompanyFdr+'/mlab/managelab',GlbSearchParam,'json',fnListLabRes);
}
        ...
        if(data.errcode!=undefined) {
            if(data.errcode == '404') {
                ...
                if(data.cn>0) {
                    ...
                    PageContent	= "<table class='table table-bordered table-hover'><thead><tr><th>Lab Name</th><th>Status</th><th>Updated By</th><th>Date Updated</th><th>Action</th></tr></thead><tbody>";
                    if(data.ct>0) {
                        $.each(data.re,function(index,value){
                            PageContent=PageContent+'<tr><td><a href="'+base_path+GlbCompanyFdr+'/mlab/addedit/'+escape(base64_encode(value.id))+'/">'+value.n+'</a></td><td>'+value.s+'</td><td>'+value.ub+'</td><td>'+value.du+'</td><td><i class="fa fa-trash-o"></i>&nbsp;&nbsp;<a href="javascript:void(0);" onclick="fnDeleteLab('+value.id+')">Delete</a></td>';
...
function fnPaginationLab(VarURL) {
    ...
    $("#DivTotalCntResult").html('');
    $("#ResResult").html("<img src='"+base_path+"/assets/img/loader.gif' height='8' style='padding-left:10px'>");
    MakePostRequest(VarURL,Parameters,'json',fnListLabRes);
}

function fnDeleteLab(Id) {
    if(confirm("Are you want to delete this lab?")) {
        var Parameters = "id="+Id;
        MakePostRequest(base_path+GlbCompanyFdr+'/mlab/delLabInfo',Parameters,'json',fnDeleteLabRes);
    }
}

function fnDeleteLabRes(data){
    if(data!=''){
        if(data.errcode!=undefined) {
            if(data.errcode == '404') {
                fnCallSessionExpire();
...
function fnSaveLabInfo() {
    try{
        ...
        if(jsTrim(LabName)== ""){
            $('#ErrBasicLabName').html("Please fill the lab");
...
function fnSaveLabRes(data){
    if(data!=''){
        if(data.errcode == '404') {
            ...
            $("#divSuccessBasicInfoMsg").html("Lab has updated at successfully!");
