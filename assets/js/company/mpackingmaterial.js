...
function fnSearchPackingMaterial() {
    ...
    MakePostRequest(base_path+GlbCompanyFdr+'/mpackingmaterial/managepackingmaterial',GlbSearchParam,'json',fnListPackingMaterialRes);
}
                ...
                if(data.cn>0) {
                    ...
                    if(data.ct>0) {
                        $.each(data.re,function(index,value){
                            PageContent=PageContent+'<tr><td><a href="'+base_path+GlbCompanyFdr+'/mpackingmaterial/addedit/'+escape(base64_encode(value.id))+'/">'+value.n+'</a></td><td>'+value.s+'</td><td>'+value.ub+'</td><td>'+value.du+'</td><td><i class="fa fa-trash-o"></i>&nbsp;&nbsp;<a href="javascript:void(0);" onclick="fnDeletePackingMaterial('+value.id+')">Delete</a></td>';
...
function fnDeletePackingMaterial(Id) {
    if(confirm("Are you want to delete this packing material?")) {
        ...
        MakePostRequest(base_path+GlbCompanyFdr+'/mpackingmaterial/delPackingMaterialInfo',Parameters,'json',fnDeletePackingMaterialRes);
    }
}
