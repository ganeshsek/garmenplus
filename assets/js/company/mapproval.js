...
function fnSearchApproval() {
    ...
    MakePostRequest(base_path+GlbCompanyFdr+'/mapproval/manageapproval',GlbSearchParam,'json',fnListApprovalRes);
}
                ...
                if(data.cn>0) {
                    ...
                    if(data.ct>0) {
                        $.each(data.re,function(index,value){
                            PageContent=PageContent+'<tr><td><a href="'+base_path+GlbCompanyFdr+'/mapproval/addedit/'+escape(base64_encode(value.id))+'/">'+value.n+'</a></td><td>'+value.s+'</td><td>'+value.ub+'</td><td>'+value.du+'</td><td><i class="fa fa-trash-o"></i>&nbsp;&nbsp;<a href="javascript:void(0);" onclick="fnDeleteApproval('+value.id+')">Delete</a></td>';
...
function fnDeleteApproval(Id) {
    if(confirm("Are you want to delete this process flow?")) {
        ...
        MakePostRequest(base_path+GlbCompanyFdr+'/mapproval/delApprovalInfo',Parameters,'json',fnDeleteApprovalRes);
    }
}
...
function fnSaveApprovalInfo() {
    try{
        ...
        if(jsTrim(ApprovalName)== ""){
            $('#ErrBasicApprovalName').html("Please fill the process flow");
        ...
        $.ajax({
            url 		: base_path+GlbCompanyFdr+'/mapproval/updateApprovalInfo',
...
function fnSaveApprovalRes(data){
    if(data!=''){
        if(data.errcode == '404') {
            ...
            fnRedirectPageTimeOut(base_path+GlbCompanyFdr+'/mapproval/addedit/'+data.eid);
        }
    }
}