...
function fnSearchPort() {
    ...
    MakePostRequest(base_path+GlbCompanyFdr+'/mport/manageport',GlbSearchParam,'json',fnListPortRes);
}
                ...
                if(data.cn>0) {
                    ...
                    if(data.ct>0) {
                        $.each(data.re,function(index,value){
                            PageContent=PageContent+'<tr><td><a href="'+base_path+GlbCompanyFdr+'/mport/addedit/'+escape(base64_encode(value.id))+'/">'+value.pn+'</a></td><td>'+value.pa+'</td><td>'+value.pc+'</td><td>'+value.pcntry+'</td><td>'+value.s+'</td><td>'+value.ub+'</td><td>'+value.du+'</td><td><i class="fa fa-trash-o"></i>&nbsp;&nbsp;<a href="javascript:void(0);" onclick="fnDeleteSizeRange('+value.id+')">Delete</a></td>';
...
function fnDeletePort(Id) {
    if(confirm("Are you want to delete this port?")) {
        ...
        MakePostRequest(base_path+GlbCompanyFdr+'/mport/delPortInfo',Parameters,'json',fnDeletePortRes);
    }
}
