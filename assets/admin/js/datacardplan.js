function fnSaveDataCardPlan() {
    ...
    $.ajax({
        url 		: base_path+'admin/recharge/updateDataCardPlanInfo',
        ...
        success     : function(data, textStatus, jqXHR){
            ...
            fnSaveDatCardPlanRes(data);
        }
    });
    return false;
}

function fnSaveDatCardPlanRes(data){
    if(data!=''){
        if(data.errcode == '404') {
            fnCallSessionExpire();
            return false;
            ...
            if(GlbPlanId>=1) {
                fnRedirectPageTimeOut(base_path+'admin/recharge/adddatacardplan/'+GlbPlanId);
            } else {
                resetForm("frmNameDataCardPlan");
            }
        }
    }
}

function fnSearchDataCardPlan() {
    ...
    MakePostRequest(base_path+'admin/recharge/managedatacardrechargeplan',GlbSearchParam,'json',fnListMobilePlanRes);
}

function fnListDataCardPlan() {
    GlbSearchParam								= "rfrom=1";
    $("#DivTotalCntResult").html('');
    $("#ResResult").html("<img src='"+base_path+"/assets/img/loader.gif' height='8' style='padding-left:10px'>");
    MakePostRequest(base_path+'admin/recharge/managedatacardrechargeplan',GlbSearchParam,'json',fnListDataCardPlanRes);
}

function fnListDataCardPlanRes(data){
    if(data!=''){
                ...
                if(data.cn>0) {
                    ...
                    if(data.ct>0) {
                        $.each(data.re,function(index,value){
                            ...
                            PageContent=PageContent+'<a href="'+base_path+'admin/recharge/adddatacardplan/'+value.id+'/"><i class="fa fa-file-text-o"></i> Edit</a>';
...
function fnPagination(VarURL) {
    ...
    $("#DivTotalCntResult").html('');
    $("#ResResult").html("<img src='"+base_path+"/assets/img/loader.gif' height='8' style='padding-left:10px'>");
    MakePostRequest(VarURL,Parameters,'json',fnListDataCardPlanRes);
}

function fnDeletePlanInfo(Id) {
    if(confirm("Are you want to delete this plan?")) {
        var Parameters = "id="+Id;
        MakePostRequest(base_path+'admin/recharge/delDataCardPlanInfo',Parameters,'json',fnDeletePlanInfoRes);
    }
}
    ...
    if(data!=''){
        if(data.errcode!=undefined) {
            if(data.errcode == '404') {
                ...
                fnSearchDataCardPlan();
            }
        }
    }
}

function fnUploadDataCardPlan() {
    $('.form-control').css("border", "1px solid #cccccc");
    $('div.herr').html('');
    var ProfileFormData							= false;
    ...
    $.ajax({
        url 		: base_path+'admin/recharge/saveUploadDataCardPlan',
        data        : ProfileFormData ? ProfileFormData : ObjForm.serialize(),
        cache       : false,
        contentType : false,
        processData : false,
        ...
        success     : function(data, textStatus, jqXHR){
            data = jQuery.parseJSON(data);
            fnUploadDataCardPlanRes(data);
        }
    });
    return false;
}

function fnUploadDataCardPlanRes(data){
    if(data!=''){
        if(data.errcode == '404') {
            fnCallSessionExpire();
            return false;
            ...
            if(GlbPlanId>=1) {
                ...
                resetForm("frmNameUploadDataCardPlan");
            }
        }
    }
}